...
export const LessonViewer: React.FC<LessonViewerProps> = ({
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 p-1.5 bg-slate-200/70 border border-slate-300/60 rounded-2xl">
        {lessonsData.map(lesson => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2.5 p-2.5 sm:p-3 rounded-xl text-left transition-all cursor-pointer border ${
                isSelected
                  ? 'bg-white text-indigo-900 border-indigo-200 shadow-sm ring-2 ring-indigo-500/20 font-bold'
                  : 'bg-transparent border-transparent text-slate-600 hover:bg-white/60 hover:text-slate-900 font-medium'
              }`}
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                isSelected ? 'bg-indigo-600 text-white shadow-xs' : 'bg-slate-300/80 text-slate-700'
              }`}>
                <Icon className="w-4 h-4" />
              </div>
              <div className="min-w-0">
                <div className="text-xs truncate flex items-center gap-1 font-bold">
      ...
      {/* 3.5. MINH HỌA & SƠ ĐỒ TRỰC QUAN TƯƠNG TÁC */}
      <div className="space-y-4">
        <div className="flex items-center gap-2 text-slate-800 font-extrabold text-lg sm:text-xl border-b border-slate-200 pb-3">
          <Eye className="w-5 h-5 text-indigo-600" />
          <span>Sơ đồ & Mô hình minh họa trực quan chuyên sâu</span>
        </div>

        {currentLesson.id === 1 && <MolecularGeometryVisualizer />}
        {currentLesson.id === 2 && <NuclearRadiationVisualizer />}
        {currentLesson.id === 3 && <ArrheniusEnergyProfileVisualizer />}
        {currentLesson.id === 4 && <ThermodynamicGibbsVisualizer />}
      </div>
              ...
              <div key={qc.id} className="bg-white rounded-2xl p-5 border border-amber-200/80 shadow-2xs space-y-4">
                ...
                <div className="space-y-2 pl-0 sm:pl-7">
                  {qc.options.map((opt, oIdx) => {
                    ...
                    let btnClass = 'border-slate-200 hover:border-amber-400 bg-slate-50/70 text-slate-700 hover:bg-amber-50/30';
                    if (isAnswered) {
                      if (isSelected && isCorrect) {
                        ...
                        btnClass = 'border-emerald-400 bg-emerald-50/80 text-emerald-900 font-semibold';
                      } else {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-3 rounded-xl border text-xs sm:text-sm transition-all flex items-center justify-between gap-3 cursor-pointer ${btnClass}`}
                      >
                        <div className="flex items-center gap-2.5">
                          <span className={`w-6 h-6 rounded-lg text-xs font-bold flex items-center justify-center shrink-0 border ${
                            isSelected ? 'bg-amber-600 text-white border-amber-600' : 'bg-white text-slate-700 border-slate-300'
                          }`}>
              ...
              <div key={q.id} className="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                ...
                <div className="space-y-2 pl-0 sm:pl-8">
                  {q.options.map((opt, oIdx) => {
                    ...
                    let btnClass = 'border-slate-200 bg-white hover:border-indigo-400 text-slate-700 hover:bg-indigo-50/20';
                    if (isSelected && !quizSubmitted) {
                      btnClass = 'border-indigo-600 bg-indigo-50/80 text-indigo-950 font-semibold ring-2 ring-indigo-500/20';
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          <span className={`w-6 h-6 rounded-lg text-xs font-bold flex items-center justify-center shrink-0 border ${
                            isSelected ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-slate-100 text-slate-700 border-slate-300'
                          }`}>
                            {String.fromCharCode(65 + oIdx)}
                          </span>
                          <span className="leading-snug">{opt}</span>
